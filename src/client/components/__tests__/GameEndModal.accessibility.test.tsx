...
describe('GameEndModal Accessibility', () => {
  ...
  test('should have proper button accessibility', async () => {
    ...
    await waitFor(() => {
      ...
      const boastButton = screen.getByRole('button', { name: /share your stonefall results to reddit/i });

      expect(tryAgainButton).toHaveAttribute('aria-label', 'Start a new game');
      expect(boastButton).toHaveAttribute('aria-label', 'Share your Stonefall results to Reddit');
